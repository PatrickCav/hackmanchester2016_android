...
public class ChallengesFragmentFriends extends ChallengesFragmentBase {

    public static final String TITLE = "Friend's Challenges";

    @Override
    protected void requestChallenges() {
        Engine.getInstance().getRetrofitClient().getApi().getFriendsChallenges().enqueue(new Callback<ChallengeResponse>() {
            @Override
    ...
    protected ChallengesAdapterBase getAdapter(ChallengesAdapterBase.ChallengesCallback callback) {
        return new ChallengesAdapterFriends(callback);
    }
}
